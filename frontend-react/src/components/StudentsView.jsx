...
function QrModal({ student, onClose }) { return <ModalDialog id="qrModal" onClose={onClose}><div className="qr-modal-content"><button className="close-btn qr-close" type="button" onClick={onClose}>×</button><p className="eyebrow">STUDENT ACCESS CARD</p><h2>{student.name}</h2><p className="muted">{student.roll} · {student.course} · {student.section}</p><div className="qr-image-wrap"><img src={qrUrl(student.token)} alt={`QR code for ${student.name}`} /></div><code>{student.token}</code><button className="primary-btn wide" type="button" onClick={() => window.print()}>▣ Print QR card</button></div></ModalDialog>; }
