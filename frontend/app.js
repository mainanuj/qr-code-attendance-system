...
function render() { renderDashboard(); renderStudents(); renderRecords(); updateCourseFilter(); updateYearFilter(); }
...
function renderStudents() {
  const term = $('#studentSearch').value.toLowerCase(); const course = $('#courseFilter').value;
  const students = state.students.filter(s => (!course || s.course === course) && `${s.name} ${s.roll}`.toLowerCase().includes(term));
  $('#studentRows').innerHTML = students.map((student, i) => `<tr><td><div class="table-student">${avatar(student,i)}<span>${escapeHtml(student.name)}</span></div></td><td>${escapeHtml(student.roll)}</td><td>${escapeHtml(student.course)}</td><td><span class="qr-issued">● ISSUED</span></td><td><div class="row-actions"><button class="small-btn" data-card="${student.id}">View QR</button><button class="small-btn danger" data-delete="${student.id}" title="Delete student">×</button></div></td></tr>`).join('');
  $('#studentsEmpty').style.display = state.students.length ? 'none' : 'block';
}
function renderRecords() {
  const date = $('#recordDate').value; const year = $('#recordYear').value; const status = $('#recordStatus').value; const term = $('#recordSearch').value.toLowerCase();
  const rows = [...state.attendance].sort((a,b) => b.createdAt-a.createdAt).filter(record => {
    const student = state.students.find(s => s.id === record.studentId) || {}; return (!date || record.date === date) && (!year || record.date.startsWith(year)) && (!status || record.status === status) && (!term || (student.name || '').toLowerCase().includes(term));
  });
  $('#recordRows').innerHTML = rows.map((record, i) => { const student = state.students.find(s => s.id === record.studentId) || {name:'Deleted student',roll:'—'}; return `<tr><td><div class="table-student">${avatar(student,i)}<span>${escapeHtml(student.name)}</span></div></td><td>${escapeHtml(student.roll)}</td><td>${formatDate(record.date)}</td><td>${record.time}</td><td><span class="status ${record.status.toLowerCase()}">${record.status.toUpperCase()}</span></td></tr>`; }).join('');
...
function showView(view) { $$('.view').forEach(v => v.classList.toggle('active', v.id === view)); $$('.nav-link').forEach(b => b.classList.toggle('active', b.dataset.view === view)); $('#pageTitle').textContent = view === 'dashboard' ? 'Good morning, Alex ✦' : ({students:'Student roster',scanner:'Scan attendance',records:'Attendance log'})[view]; if (view !== 'scanner') stopCamera(); }
...
function showCard(student) { selectedStudent = student; $('#qrName').textContent=student.name; $('#qrDetails').textContent=`${student.roll} · ${student.course}`; $('#qrToken').textContent=student.token; $('#qrImage').src=`https://api.qrserver.com/v1/create-qr-code/?size=320x320&margin=0&data=${encodeURIComponent(student.token)}`; $('#qrModal').showModal(); }
function addStudent(event) { event.preventDefault(); const student={id:crypto.randomUUID(),name:$('#studentName').value.trim(),roll:$('#studentRoll').value.trim(),course:$('#studentCourse').value.trim(),token:token()}; if(state.students.some(s=>s.roll.toLowerCase()===student.roll.toLowerCase())) {toast('That roll number already exists.');return;} state.students.push(student);saveState();$('#studentModal').close();event.target.reset();render();showCard(student);toast(`${student.name} was added and issued a QR card.`); }
...
function exportCsv() { const rows=[['Student','Roll number','Course','Date','Check-in time','Status'],...state.attendance.map(r=>{const s=state.students.find(x=>x.id===r.studentId)||{};return[s.name||'Deleted student',s.roll||'',s.course||'',r.date,r.time,r.status];})];const csv=rows.map(row=>row.map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')).join('\n');const url=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));const a=document.createElement('a');a.href=url;a.download=`attendly-${today()}.csv`;a.click();URL.revokeObjectURL(url);toast('Attendance CSV downloaded.');}
...
$('#studentSearch').addEventListener('input',renderStudents);$('#courseFilter').addEventListener('change',renderStudents);
$('#recordDate').value=today();$('#recordDate').addEventListener('change',renderRecords);$('#recordYear').addEventListener('change',()=>{if($('#recordYear').value)$('#recordDate').value='';renderRecords();});$('#recordStatus').addEventListener('change',renderRecords);$('#recordSearch').addEventListener('input',renderRecords);
